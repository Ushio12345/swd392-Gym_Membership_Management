...
const Avatar = React.forwardRef<React.ComponentRef<typeof AvatarPrimitive.Root>, AvatarProps>(({ className, variant = "prime", ...props }, ref) => {
    ...
    return (
        <div className={cn("inline-flex rounded-full", withBorder)}>
            <AvatarPrimitive.Root
                ref={ref}
                className={cn("flex h-10 w-10 shrink-0 overflow-hidden rounded-full bg-background", className)}
                {...props}
            />
        </div>
    );
