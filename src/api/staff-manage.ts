...
export const fetchPackages = async () => {
  try {
    ...
    if (error?.response?.status === 404) {
      throw new Error("API endpoint khÃ´ng tá»“n táº¡i. Vui lÃ²ng kiá»ƒm tra láº¡i.")
    } else if (error?.response?.status === 401) {
      throw new Error("Báº¡n cáº§n Ä‘Äƒng nháº­p Ä‘á»ƒ truy cáº­p chá»©c nÄƒng nÃ y.")
...
export const fetchServicesByCenter = async (centerId: number) => {
  try {
    ...
    const res = await axiosInstance.get("/staff/services/active")
    const allServices = res.data || res
    ...
    const centerServices = allServices.filter((service: any) => service.centerId === centerId)
